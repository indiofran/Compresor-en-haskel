...
pixelsDiferentesEnFrame ((f:[]):cs) ((f2:[]):cs2) u | norma (diferenciaPixel f f2) > u = [(getFila (26 - toInteger(length(encadenar ((f:[]):cs)))), getCol (26 - toInteger(length(encadenar ((f2:[]):cs2)))), diferenciaPixel f f2)] ++ pixelsDiferentesEnFrame cs cs2 u
                                                    | otherwise = pixelsDiferentesEnFrame cs cs2 u
pixelsDiferentesEnFrame ((f:fs):cs) ((f2:fs2):cs2) u | norma (diferenciaPixel f f2) > u = [(getFila (26 - toInteger(length(encadenar ((f:fs):cs)))), getCol (26 - toInteger(length(encadenar ((f2:fs2):cs2)))), diferenciaPixel f f2)] ++ pixelsDiferentesEnFrame ((fs):cs) ((fs2):cs2) u

                                                     | otherwise = pixelsDiferentesEnFrame ((fs):cs) ((fs2):cs2) u
--pixelsDiferentesEnFrame ((f:[]):cs) ((f2:[]):cs2) u | norma (diferenciaPixel f f2) > u = [(getCol (26 - posicionLista ((f:[]):cs)), getFila (26 - posicionLista ((f:[]):cs)), diferenciaPixel f f2)] ++ pixelsDiferentesEnFrame cs cs2 u
--                                                    | otherwise = pixelsDiferentesEnFrame cs cs2 u
--pixelsDiferentesEnFrame ((f:fs):cs) ((f2:fs2):cs2) u | norma (diferenciaPixel f f2) > u = [(getCol (26 - posicionLista ((f:[]):cs)), getFila (26 - posicionLista ((f:[]):cs)), diferenciaPixel f f2)] ++ pixelsDiferentesEnFrame ((fs):cs) ((fs2):cs2) u
--                                                     | otherwise = pixelsDiferentesEnFrame ((fs):cs) ((fs2):cs2) u


getFila :: Integer -> Integer
getFila x | x < 6 = 0
          | (mod x 5) == 0 = (div x 5) - 1
          | otherwise = div x 5 

getCol :: Integer -> Integer
getCol x | x < 6 = x - 1
         | (mod x 5) == 0 = 4
         | otherwise = (mod x 5) - 1

encadenar :: Frame -> [Pixel]
encadenar [] = []
encadenar (x:xs) = x ++ encadenar xs
    
diferenciaPixel :: Pixel -> Pixel -> (Integer, Integer, Integer)
diferenciaPixel (x1, x2, x3) (y1, y2, y3) = (x1 - y1, x2 - y2, x3 - y3)
...
comprimir (Agregar f v) u n = comprimir v u n

frameSiguiente :: Video -> Frame

cantidadDeDiferencias :: Frame -> Frame -> Float ->  Integer
cantidadDeDiferencias ((f:fs):cs) ((f2:fs2):cs2) u | norma (diferenciaPixel f f2) > u = 1 + cantidadDeDiferencias (((fs):cs) ((f2):cs2) u)
                                                   | otherwise = cantidadDeDiferencias (((fs):cs) ((f2):cs2) u)
